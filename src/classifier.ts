...
export type ClassificationStatus = 'VERIFIED' | 'VERIFIED_WITH_CORRECTIONS' | 'FLAGGED';
...
export function classify(
    diffsBySource: Map<string, { tier: number, diffs: FieldDiff[], hasStrongIdentifierMatch: boolean }>,
    ...
    for (const [sourceId, { tier, diffs, hasStrongIdentifierMatch }] of diffsBySource.entries()) {
        ...
        const mismatches = diffs.filter(d => d.status === 'mismatch' || d.status === 'missing-zotero');
        const criticalMismatches = mismatches.filter(d => criticalFields.includes(d.field));

        if (criticalMismatches.length === 0 && hasStrongIdentifierMatch) {
            exactPrimaryMatches++;
        } else if (hasStrongIdentifierMatch || (diffs.find(d => d.field === 'title')?.status === 'match' && diffs.find(d => d.field === 'authors')?.status === 'match')) {
