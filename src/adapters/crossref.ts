...
export class CrossrefAdapter implements SourceAdapter {
    ...
    private getHeaders(prefs: PluginPrefs) {
        ...
        if (email) {
            headers['User-Agent'] = `ZoteroReferenceValidator/0.1.0 (mailto:${email})`;
        }
        return headers;
    }
    ...
    async getById(identifier: Identifier, prefs: PluginPrefs = {}): Promise<CanonicalRecord | null> {
        ...
        try {
            const response = await fetch(`https://api.crossref.org/works/${encodeURIComponent(identifier.doi)}`, {
    ...
    async search(query: SearchQuery, prefs: PluginPrefs = {}): Promise<CanonicalRecord[]> {
        ...
        try {
             ...
             const data = await response.json();
             return (data.message.items || []).map((item: any) => this.normalize(item)).slice(0, 5);
        } catch (e) {
            return [];
        }
    }
